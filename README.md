...
El panorama regional precarga un dataset determinista de doce hospitales y setenta y dos equipos ficticios en cinco países (Panamá, Brasil, Colombia, México y Chile). Resume hospitales, equipos, confianza, información desactualizada y oportunidades potenciales; permite filtrar por cliente, hospital, región, país, ciudad y modalidad. El mapa vectorial destaca únicamente los tres países principales (Panamá, Brasil, Colombia); los demás países funcionan igual en filtros, listas y agregaciones, solo sin una forma en el mapa. Cada país se agrupa además bajo una Región (hoy "América Latina" para todos), siguiendo la jerarquía Región → País → Ciudad → Cliente. El mapa vectorial, los gráficos y los demás recursos son locales. Las capturas guardadas se incorporan al panorama y se distinguen del dataset precargado; restablecer la demostración no elimina esas capturas.
...
La preparación (`npm ci`, `npx qvac doctor`, cada `npm run qvac:prepare:*`) requiere conexión y descarga los pesos de los modelos una sola vez a la caché local de QVAC (`~/.qvac/models`); documenta la ruta exacta que cada script imprime. El arranque normal (`npm start` / `Start-SiteSignal.ps1`) no descarga nada: solo lee esas rutas locales vía `SITESIGNAL_MODEL`, `SITESIGNAL_VOICE_MODEL` y `SITESIGNAL_PLATE_MODEL`. Ningún archivo de `src/` ni `public/` hace una petición a un host que no sea `127.0.0.1`; los únicos consumidores de red son los scripts de preparación y verificación (`qvac:prepare:*`, `qvac:check:*`), nunca la aplicación en sí.

## Ejecución sin conexión

Para comprobar que la aplicación funciona sin red una vez preparados los modelos: prepara los tres modelos con conexión, desconéctala (apaga el adaptador de red o bloquea la salida con el Firewall de Windows) y luego ejecuta `npm start` con las tres variables de entorno configuradas. Con la red desactivada deberían funcionar sin error: registrar y revisar una observación escrita, dictarla por voz, adjuntar y analizar una evidencia fotográfica, navegar el panorama regional y el perfil 360 de un hospital, interpretar una consulta natural y descargar ambas exportaciones. Como QVAC corre en un proceso hijo local y los tres modelos ya están en caché, ninguno de esos pasos depende de la red; los únicos comandos que sí la requieren (`qvac:prepare:*`, `qvac:check:*`, `npm ci`) quedan fuera de esta prueba por diseño.
...
Dependencias de código: `@qvac/sdk` (motor de inferencia local), `zod` (validación de esquemas) y `sharp` (procesamiento de imágenes 100% local, usado solo para ampliar una foto de evidencia pequeña antes del OCR — no envía ni recibe nada por red); sin frameworks de frontend ni librerías de terceros en `public/` — HTML, CSS y JavaScript nativo servidos únicamente desde este mismo origen. Modelos QVAC usados: Qwen3 4B Instruct Q4_K_M (extracción de texto y consultas naturales), Whisper small Q8_0 multilingüe (dictado de voz), EasyOCR (`latin_g2` + detector `craft_mlt_25k`, evidencia fotográfica). Hardware de referencia con el que se probó este prototipo: AMD Ryzen 7 8845HS (8 núcleos físicos / 16 lógicos), 16 GB de RAM, GPU integrada AMD Radeon 780M — toda la inferencia corrió sobre CPU. La sección "Estado del entorno" de la aplicación declara el hardware detectado en cada equipo donde se ejecute.
