...
## Correcciones de usabilidad tras la primera prueba del rediseño

El usuario probó el rediseño y señaló cinco puntos, todos corregidos y verificados con QVAC real y pruebas automatizadas (100/100):

- **Reproducción del dictado**: al detener una grabación, el audio quedaba inaudible. Causa real: la política de seguridad de contenido (CSP) no declaraba `media-src`, así que heredaba `default-src 'self'` — y `'self'` no cubre el esquema `blob:` con el que `voice.js` arma la URL del audio grabado (`URL.createObjectURL`), aunque `img-src` sí lo permitiera para otros usos. El navegador rechazaba la carga con "Media load rejected by URL safety check". Corregido en `src/application.js` agregando `media-src 'self' blob:` a la cabecera. Verificado inyectando un blob de audio de prueba antes y después del cambio: el error cambió de un bloqueo de seguridad a un error de decodificación normal (el blob de prueba no era un WAV válido), confirmando que la política ya no es la causa.
- **Cambio de pestaña tras transcribir**: `voice.js` llenaba el cuadro de texto pero dejaba al colaborador en la pestaña "Dictar", donde no veía la transcripción. Se agregó el evento `sitesignal:capture-tab` (disparado por `voice.js`, escuchado por `public/router.js` vía una función `activateCaptureTab()` extraída del manejador de clic existente) que cambia a la pestaña "Escribir" automáticamente tras una transcripción exitosa.
- **Etiqueta de grupo dinámica**: la tarjeta de un grupo de equipos en el perfil 360 (`public/capture.js`, función `showHospital`) mostraba la insignia genérica "GRUPO DE EQUIPOS · 2"; ahora nombra la modalidad real: "CANTIDAD DE TOMOGRAFÍA COMPUTARIZADA · 2". La pregunta de seguimiento sobre cantidad, fabricante, modelo o antigüedad (`src/confidence.js`, `nextFollowUp`) también se volvió dinámica: si la modalidad del equipo ya se conoce, pregunta "¿Cuántos equipos de Tomografía computarizada observaste?" en vez de "¿Cuántos equipos observaste? Equipo 2."
- **Pestañas del perfil 360**: se verificaron las cinco (Base instalada, Observaciones, Pendientes, Oportunidades, Historial) haciendo clic real en cada una sobre un hospital capturado con candidatos, oportunidades y equipos — todas cambian de contenido correctamente y las acciones (corregir dato, consolidar, resolver conflicto) funcionan. No se encontró ningún defecto; la sospecha inicial pudo deberse a la caché de módulos ya documentada arriba, o a que las acciones de corrección solo aparecen en hospitales capturados, no en los ficticios del dataset precargado.
- **Selección automática de destino**: cuando el nombre del hospital, cliente, ciudad y país del borrador ya coinciden exactamente con un hospital existente (la misma regla que desde antes rechaza con 409 crear un duplicado al guardar), el selector "Destino de la observación" ahora se autoselecciona ese hospital en vez de dejarlo en blanco para elección manual. Un candidato más débil (coincidencia parcial del nombre únicamente) sigue exigiendo elección humana, mostrado como "Posible coincidencia"; la coincidencia exacta se distingue con la etiqueta "Coincidencia exacta". Implementado compartiendo una sola función `findExactHospitalMatch()` en `src/observations.js` entre `presentDraft()` (expone `draft.exactMatchId`) y el chequeo de rechazo de duplicados en `POST /api/observations`, que antes duplicaba esta misma lógica inline.

`test/hospital-duplicate-prevention.test.js` ganó dos pruebas para `exactMatchId` (coincidencia exacta selecciona el hospital; coincidencia parcial no). Verificado en vivo con QVAC real: una segunda observación para "Hospital Rediseño Ficticio" con el mismo cliente y ciudad se guardó directamente sobre el hospital existente sin ningún error, con el selector ya en "Coincidencia exacta · Hospital Rediseño Ficticio · Red Diseño Ficticia" al llegar al paso de revisión.

## Segunda ronda: el usuario probó las correcciones anteriores y encontró dos fallas reales en ellas

- **El cambio de pestaña tras transcribir usaba el identificador equivocado**: el evento `sitesignal:capture-tab` se disparaba con `detail: 'text'`, pero los atributos reales en `public/index.html` son `data-capture-tab="write"` / `data-capture-tab="voice"` (nunca `"text"`). Como ningún botón coincidía con `'text'`, `activateCaptureTab()` no marcaba ningún botón activo **y ocultaba ambos paneles** (Escribir y Dictar), dejando la pantalla en blanco bajo la barra de pestañas — exactamente "trata de irse a la pestaña de escribir pero no lo termina de hacer". Corregido cambiando el detail a `'write'` en `public/voice.js`. Verificado disparando el evento directamente en el navegador con QVAC real: antes y después, confirmando que ambos paneles quedan correctamente mostrados/ocultos y el botón "Escribir" queda con `.active`.
- **La coincidencia exacta de hospital no toleraba la palabra genérica "Hospital"**: el usuario reportó que la app seguía dejando crear "Santo Tomás" como un segundo hospital distinto de "Hospital Santo Tomás", sin autoseleccionar ninguno — confirmado como una brecha real, no solo una impresión: `findExactHospitalMatch()` comparaba nombres con `normalize()` puro (solo acentos/mayúsculas), así que "Hospital Santo Tomás" y "Santo Tomás" nunca eran el mismo string normalizado. QVAC extrae el campo `hospital` con variabilidad real en si incluye o no la palabra de tipo de institución entre una visita y otra, un caso ya documentado en este archivo para otros campos. Corrección: `src/observation-schema.js` gana `hospitalCoreName()` (exportada, reutiliza la misma lista `institutionTypeWords` que ya usaba `hospitalLabel` para el anclaje de extracción) que quita una palabra de tipo de institución (hospital/clínica/instituto/policlínica/sanatorio/centro médico/centro de salud, en español e inglés) antes de comparar, con resguardo explícito para no dejar un nombre vacío si el relato dice solo "Hospital" a secas. `findExactHospitalMatch()` en `src/observations.js` ahora compara por este nombre "núcleo" en vez del nombre normalizado literal, beneficiando tanto a `draft.exactMatchId` como al rechazo 409 existente al guardar (ambos comparten la misma función, así que se corrigen juntos).
- `test/hospital-duplicate-prevention.test.js` ganó una prueba de regresión exacta para este caso ("Hospital Santo Tomás" vs. "Santo Tomás": la segunda visita trae `exactMatchId` apuntando al primero, se guarda sobre el mismo hospital sin fricción, y un tercer intento de "crear un hospital nuevo" con ese mismo nombre sigue rechazándose con 409). Verificado: `npm test` 101/101, `npm run typecheck` limpio, `git diff --check` limpio.

## Catálogo de equipos más realista y base local reiniciada a cero

El usuario pidió dos cosas: (1) que el dataset ficticio precargado usara datos "más realistas" porque muchos campos se veían genéricos/sin definir, y (2) borrar los hospitales que se crearon durante las sesiones de prueba para arrancar de cero.

- **Catálogo de equipos**: `src/regional-panorama.js` ganó `equipmentCatalog`, un mapa de modalidad → lista de pares `[fabricante, modelo]` con marcas reales del sector médico (Philips, GE Healthcare, Siemens Healthineers, Canon Medical, Mindray, ZOLL) y modelos comerciales plausibles por modalidad (p. ej. resonancia magnética → Philips Ingenia Elition X / GE SIGNA Premier / Siemens MAGNETOM Sola / Canon Vantage Galan 3T). Reemplaza los antiguos `Fabricante Ficticio 1/2/3` y `Modelo Ficticio A-H`, que se repetían idénticos en las setenta y dos filas sin relación con la modalidad real del equipo. El hospital, cliente y colaboradores del dataset siguen siendo 100% ficticios — la regla "sin datos reales de pacientes o clientes" nunca aplicó a marcas públicas de fabricantes de equipos, y usarlas hace que la demostración se vea como una base instalada real en vez de con marcadores genéricos. `DATASET_ID` subió a `v4` para forzar el reseed en cualquier base existente. Ningún test comprobaba los valores literales del catálogo anterior, así que no hubo que tocar ninguna prueba; las 101 existentes siguen pasando.
- **Base local reiniciada a cero**: se detuvo el servidor y se borró `data/sitesignal.db` (no había `data/evidence/*` que limpiar; las evidencias fotográficas se guardan dentro de SQLite, no como archivos sueltos). Al reiniciar, la aplicación recreó la base desde cero (nuevo `installationId`, `starts: 1`) y resembró automáticamente los doce hospitales ficticios del dataset precargado con el catálogo nuevo — `/api/hospitals` (captura local) devuelve `[]` y `/api/panorama` solo lista los doce hospitales ficticios, ninguno de los creados durante las pruebas de sesiones anteriores (Repro*, DemoCare Oeste, Rediseño Ficticio, Puente Ficticio, Santo Tomás, San Gabriel, etc.).
- `README.md` documenta la decisión del catálogo de fabricantes reales en la sección del panorama regional.

Verificado: `npm test` 101/101, `npm run typecheck` limpio, `git diff --check` limpio, y confirmado en vivo contra el servidor real: doce hospitales / setenta y dos equipos, cada equipo con fabricante y modelo coherentes con su modalidad, y la lista de hospitales capturados vacía tras el reinicio.
...
La aplicación escucha en `http://127.0.0.1:3210`. Si el puerto está ocupado, comprueba primero si ya existe una instancia válida antes de detener el proceso. La base persistente está en `data/sitesignal.db`. Las pruebas usan directorios temporales y no deben modificarla; fuera de pruebas, no la borres salvo que el usuario lo pida explícitamente (ver "Catálogo de equipos más realista y base local reiniciada a cero" más arriba: se borró una vez, a pedido explícito, para eliminar los hospitales creados durante las pruebas de sesiones anteriores).

Estado verificado al escribir este documento:

- `npm test`: 101/101 pruebas aprobadas.
- `npm run typecheck`: aprobado.
- `git diff --check`: aprobado.
- Base de datos local reiniciada a cero a pedido del usuario: `data/sitesignal.db` se recreó desde cero (nuevo `installationId`, `starts: 1`); solo contiene el dataset ficticio precargado (doce hospitales, setenta y dos equipos, catálogo de fabricantes real por modalidad) y ningún hospital capturado (`/api/hospitals` devuelve `[]`).
